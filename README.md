...
## 匿名ログインからGoogleログインへ移行する際の運用方針（本番対応メモ）
- 匿名ログイン中に作成したデータを、Googleログイン後も安全に引き継げる設計を維持する。
- `ownerUid` 不一致を診断画面で検知し、保存失敗理由を確認できるようにする。
- ユーザー確認なしで `ownerUid` を自動移行しない（自動一括変更しない）。
- 管理者向けに、`cats / publicCats / records / publicFoodRecords` の復旧手順を整備する。
